...
class RollSchool extends StatefulWidget {
  static String id='roll_school';

  @override
  State<RollSchool> createState() => _RollSchoolState();
}

class _RollSchoolState extends State<RollSchool> {
  late Future<List<RollHonourApi>> fut_inst;
  List<RollHonourApi> postList = [];
  String _selectedValue = '1';
  Future<List<RollHonourApi>> getPostApi() async{
    postList = [];
    final response = await http.get(Uri.parse('https://morehost.co.in/app/native_app/roll_school.php'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text('Lions Public School Chairperson', style: TextStyle(color: Colors.white, fontSize: 14.0),),
          actions: [
            CommotteeMenu()
          ],
        ),
