...
class RollDiagnostic extends StatefulWidget {
  static String id='roll_diagnostic';
   @override
  State<RollDiagnostic> createState() => _RollDiagnosticState();
}

class _RollDiagnosticState extends State<RollDiagnostic> {
  late Future<List<RollHonourApi>> fut_inst;
  List<RollHonourApi> postList = [];
  String _selectedValue = '1';
  Future<List<RollHonourApi>> getPostApi() async{
    postList = [];
    final response = await http.get(Uri.parse('https://morehost.co.in/app/native_app/roll_diagnostic.php'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text('Diagnostic Centre Chairperson', style: TextStyle(color: Colors.white, fontSize: 12.0),),
          actions: [
            CommotteeMenu()
          ],
        ),
