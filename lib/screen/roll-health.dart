...
class RollHealth extends StatefulWidget {
  static String id='roll_health';

  @override
  State<RollHealth> createState() => _RollHealthState();
}

class _RollHealthState extends State<RollHealth> {
  late Future<List<RollHonourApi>> fut_inst;
  List<RollHonourApi> postList = [];
  String _selectedValue = '1';
  Future<List<RollHonourApi>> getPostApi() async{
    postList = [];
    final response = await http.get(Uri.parse('https://morehost.co.in/app/native_app/roll_health.php'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text('Health Committee Chairperson', style: TextStyle(color: Colors.white, fontSize: 16.0),),
          actions: [
            CommotteeMenu()
          ],
        ),
