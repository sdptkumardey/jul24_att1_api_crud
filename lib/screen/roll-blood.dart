...
class RollBlood extends StatefulWidget {
  static String id='roll_blood';
  @override
  State<RollBlood> createState() => _RollBloodState();
}

class _RollBloodState extends State<RollBlood> {
  late Future<List<RollHonourApi>> fut_inst;
  List<RollHonourApi> postList = [];
  String _selectedValue = '1';
  Future<List<RollHonourApi>> getPostApi() async{
    postList = [];
    final response = await http.get(Uri.parse('https://morehost.co.in/app/native_app/roll_blood.php'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          ...
          title: Text('Blood Bank Chairperson', style: TextStyle(color: Colors.white),),
          actions: [
            CommotteeMenu()
          ],
        ),
